...
import { ClipboardList, Plus, Trash2, Calendar, ArrowLeft, Pencil } from 'lucide-react';
...
export const VisitSummary: React.FC<VisitSummaryProps> = ({ projects, onBack }) => {
    ...
    const sortedRows = [...rows, ...externalRows].sort((a, b) => {
            ...
            <div className="flex items-center gap-4 mb-8">
                ...
                <div className="flex-1">
                    <h1 className="text-3xl font-bold text-slate-900 flex items-center gap-3">
                        <ClipboardList className="w-8 h-8 text-blue-600"/> Riepilogo Appalti e Visite
                    </h1>
                </div>
                <button 
                    ...
                    <table className="w-full text-left border-collapse min-w-[1400px]">
                        ...
                        <tbody className="divide-y divide-slate-100 text-[11px]">
                            {sortedRows.map((row, idx) => (
                                <tr key={row.id} className={`hover:bg-blue-50/30 transition-colors ${row.isExternal ? 'bg-amber-50/10' : ''}`}>
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-3xl p-8 max-w-2xl w-full shadow-2xl">
                        <h3 className="text-2xl font-bold mb-6 text-slate-900">
                            {editingId ? 'Modifica Incarico' : 'Nuovo Incarico'}
                        </h3>
                        <div className="grid grid-cols-2 gap-5">
                            <div className="col-span-2">
                                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Ente Committente</label>
                                <input type="text" className="w-full p-3 border border-slate-200 rounded-xl outline-none font-bold uppercase" value={newEvent.entity} onChange={e => setNewEvent({...newEvent, entity: e.target.value})} />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Città</label>
                                <input type="text" className="w-full p-3 border border-slate-200 rounded-xl outline-none" value={newEvent.city} onChange={e => setNewEvent({...newEvent, city: e.target.value})} />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Incarico</label>
                                <input type="text" className="w-full p-3 border border-slate-200 rounded-xl outline-none" value={newEvent.assignment} onChange={e => setNewEvent({...newEvent, assignment: e.target.value})} />
                            </div>
                            <div className="col-span-2">
                                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Oggetto Lavori</label>
                                <textarea className="w-full p-3 border border-slate-200 rounded-xl h-24 outline-none text-sm" value={newEvent.projectName} onChange={e => setNewEvent({...newEvent, projectName: e.target.value})} />
                            </div>
                            <div className="col-span-2">
                                <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Date (YYYY-MM-DD, una per riga)</label>
                                <textarea 
                                    className="w-full p-3 border border-slate-300 rounded-xl h-32 outline-none font-mono text-sm" 
                        ...
                        <div className="flex gap-4 mt-8">
                            <button onClick={() => setShowModal(false)} className="flex-1 py-3 font-bold">Annulla</button>
                            <button onClick={handleSaveExternal} className="flex-1 py-3 bg-blue-600 text-white font-bold rounded-xl">Salva</button>
                        </div>
                    </div>
                </div>
            )}
