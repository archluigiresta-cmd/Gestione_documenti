...
export const VisitSummary: React.FC<VisitSummaryProps> = ({ projects, onBack }) => {
    ...
    // Uniamo i dati: priorità agli appalti in App, poi quelli esterni
    const rows = [
        ...projects.map(p => {
            const projectDocs = allDocs.filter(d => d.projectId === p.id).sort((a, b) => a.visitNumber - b.visitNumber);
            return {
                id: p.id,
                entity: p.entity,
                city: p.location,
                projectName: p.projectName,
                assignment: 'Incarico in App',
        ...
        <div className="max-w-[98%] mx-auto p-6 animate-in fade-in">
            <div className="flex items-center gap-4 mb-8">
                ...
                <div className="flex-1">
                    ...
                    <p className="text-slate-500 text-sm">Monitoraggio centralizzato di tutti gli incarichi (Dati Tabulati).</p>
                </div>
                <button 
                ...
                >
                    <Plus className="w-5 h-5"/> Aggiungi Appalto Esterno
                </button>
            </div>
                    ...
                    <table className="w-full text-left border-collapse min-w-[1200px]">
                        <thead className="bg-slate-50 border-b border-slate-200">
                            <tr className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                                ...
                                <th className="p-4 border-r w-48">Ente Committente</th>
                                <th className="p-4 border-r w-32">Città</th>
                                <th className="p-4 border-r w-64">Oggetto</th>
                                <th className="p-4 border-r w-48">Incarico</th>
                                {[...Array(12)].map((_, i) => (
                                    <th key={i} className="p-2 border-r text-center w-24">Visita {i+1}</th>
                                ))}
                            ...
                            {rows.map((row, idx) => (
                                <tr key={idx} className={`hover:bg-blue-50/30 transition-colors ${row.isExternal ? 'bg-amber-50/10' : ''}`}>
                                    ...
                                    <td className="p-4 border-r">
                                        <div className="font-bold text-slate-700 line-clamp-2 leading-tight">{row.projectName}</div>
                                    </td>
                                            ...
                                            <td key={i} className={`p-2 border-r text-center ${date ? 'bg-blue-50/50' : ''}`}>
                                                {date ? (
                                                    <span className="font-bold text-blue-700">
                                                        {new Date(date).toLocaleDateString('it-IT', {day:'2-digit', month:'2-digit', year:'2-digit'})}
                                                    </span>
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in zoom-in-95 duration-200">
                    <div className="bg-white rounded-3xl p-8 max-w-2xl w-full shadow-2xl border border-slate-200">
                        <h3 className="text-2xl font-bold mb-6 text-slate-900 flex items-center gap-2">
                            {editingId ? 'Modifica Appalto Esterno' : 'Nuovo Appalto Esterno'}
                        </h3>
                        <div className="grid grid-cols-2 gap-5">
                            ...
                            <div className="col-span-2">
                                <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Date Sopralluoghi (Formato YYYY-MM-DD, una per riga)</label>
                                <textarea 
                        ...
                        <div className="flex gap-4 mt-8">
                            ...
                            <button onClick={handleSaveExternal} className="flex-1 py-3 bg-blue-600 text-white font-bold rounded-xl shadow-lg hover:bg-blue-700 transition-all">Salva Appalto</button>
                        </div>
                    </div>
                </div>
            )}
