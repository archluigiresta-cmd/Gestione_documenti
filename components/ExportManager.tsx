  ...
  const handleDownloadWord = () => {
    ...
    const html = `
      ...
      <style>
        ...
        td { border: 1pt solid black; padding: 5pt; vertical-align: top; }
  ...
  if (documents.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-20 bg-white rounded-2xl border-2 border-dashed">
        ...
        <h3 className="text-xl font-bold">Nessun verbale presente</h3>
      </div>
    );
  }
    ...
    <div className="flex h-[calc(100vh-6rem)] gap-6">
      <div className="w-72 flex flex-col bg-white rounded-xl border overflow-hidden no-print shadow-sm">
        <div className="p-4 border-b bg-slate-50">
          <h3 className="font-bold flex items-center gap-2 text-sm"><FileCheck className="w-4 h-4 text-blue-600"/> Archivio Verbali</h3>
          <div className="mt-3 relative">
            <Search className="absolute left-3 top-2.5 w-4 h-4 text-slate-400"/>
            <input type="text" placeholder="Filtra..." className="w-full pl-9 pr-3 py-2 text-xs border rounded-lg" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          </div>
        </div>
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {filteredDocs.map(doc => (
            <button key={doc.id} onClick={() => onSelectDocument(doc.id)} className={`w-full text-left p-3 rounded-lg border transition-all ${currentDocId === doc.id ? 'bg-blue-50 border-blue-500' : 'bg-white hover:border-blue-200'}`}>
              <span className="text-[10px] font-bold uppercase text-slate-500">Visita di Collaudo N. {doc.visitNumber}</span>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        <div className="bg-white p-4 rounded-xl shadow-sm border mb-4 no-print flex items-center justify-between">
          <div className="flex items-center gap-3"><Gavel className="w-5 h-5 text-blue-600"/><h4 className="font-bold">Anteprima Documento</h4></div>
          <div className="flex gap-2">
            ...
            <button onClick={() => window.print()} className="bg-slate-900 text-white px-4 py-2 rounded-lg flex items-center gap-2 font-bold text-sm"><Printer className="w-4 h-4" /> PDF / Stampa</button>
          </div>
        </div>
