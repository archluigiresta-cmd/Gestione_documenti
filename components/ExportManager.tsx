...
export const ExportManager: React.FC<ExportManagerProps> = ({
  ...
  // Trova il documento corrente in modo sicuro
  ...
  const getDocLabel = (doc: DocumentVariables) => {
      switch(doc.type) {
          ...
          default: return (doc.type || 'Documento').replace(/_/g, ' ');
      }
  };
  ...
  if (documents.length === 0) {
    return (
        <div className="flex flex-col items-center justify-center p-20 bg-white rounded-2xl border-2 border-dashed border-slate-200">
            ...
            <p className="text-slate-500 text-center max-w-md">Non ci sono ancora documenti salvati per questo appalto. Crea un documento nella sezione "Collaudo".</p>
        </div>
    );
  }
  ...
  return (
    <div className="flex h-[calc(100vh-6rem)] gap-6 pb-4">
      ...
      <div className="w-72 flex flex-col bg-slate-50 rounded-xl border border-slate-200 overflow-hidden no-print shrink-0">
          <div className="p-4 border-b border-slate-200 bg-white">
              ...
              <div className="relative">
                  ...
                  <input 
                    type="text" 
                    placeholder="Cerca per data o numero..." 
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
                ...
                <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 mb-4 no-print flex items-center justify-between gap-4 shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-500 mt-1">Stato: Pronto per la stampa</p>
                        </div>
                    </div>
          ...
          {!currentDoc && (
              <div className="flex-1 flex items-center justify-center bg-slate-100 rounded-xl border-2 border-dashed border-slate-200">
                  <p className="text-slate-400">Seleziona un documento dall'archivio a sinistra.</p>
              </div>
          )}
      </div>
    </div>
