...
import { Printer, FileCheck, FileDown, Search, FileText, Calendar, ChevronRight, Trash2, Pencil } from 'lucide-react';
      ...
      <div className="w-80 flex flex-col bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden no-print shrink-0">
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-2">
              {filteredDocs.map(doc => (
                  <div
                      key={doc.id}
                      className={`w-full p-3 rounded-lg border transition-all group relative ${
                          currentDocId === doc.id 
                  ...
                  >
                      <div className="flex justify-between items-start mb-1">
                          <span 
                            ...
                            className={`text-xs font-bold uppercase tracking-wider cursor-pointer ${currentDocId === doc.id ? 'text-blue-700' : 'text-slate-500'}`}>
                              Verbale N. {doc.visitNumber}
                          </span>
                          <div className="flex items-center gap-1">
                               {currentDocId === doc.id && <div className="w-2 h-2 bg-blue-500 rounded-full animate-pulse mr-1"></div>}
                               {onEdit && (
                                   <button 
                      ...
                      <div 
                        onClick={() => onSelectDocument(doc.id)}
                        className="flex items-center gap-2 text-sm font-semibold text-slate-800 mb-1 cursor-pointer">
                          <Calendar className="w-3.5 h-3.5 text-slate-400"/>
                          {new Date(doc.date).toLocaleDateString('it-IT')}
                      </div>
                      <div 
                        onClick={() => onSelectDocument(doc.id)}
                        className="text-xs text-slate-400 truncate flex items-center gap-1 cursor-pointer">
                      ...
                      {currentDocId === doc.id && (
                          <ChevronRight 
                            onClick={() => onSelectDocument(doc.id)}
                            className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 text-blue-400 cursor-pointer"/>
                      )}
                  </div>
              ))}
