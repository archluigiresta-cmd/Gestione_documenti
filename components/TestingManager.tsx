...
import { Gavel, Plus, Trash2, Calendar, Clock, Users, ListChecks, Wand2, Loader2, Save, Mail, FileText, ChevronRight, CheckSquare, MessageSquare, Briefcase, Layout } from 'lucide-react';
...
export const TestingManager: React.FC<TestingManagerProps> = ({
  ...
  useEffect(() => {
    if (currentDoc && !currentDoc.worksIntroText) {
      const sortedVerbali = [...documents].filter(d => d.type === 'VERBALE_COLLAUDO' && d.visitNumber < currentDoc.visitNumber).sort((a,b) => b.visitNumber - a.visitNumber);
      const last = sortedVerbali[0];
      const refDate = last ? last.date : project.executionPhase.deliveryDate;
  ...
  if (!currentDoc) {
    return (
      <div className="flex flex-col items-center justify-center py-20 bg-white rounded-2xl border-2 border-dashed border-slate-200">
        ...
        <button onClick={() => onNewDocument('VERBALE_COLLAUDO')} className="mt-6 bg-blue-600 text-white px-8 py-3 rounded-xl font-bold shadow-lg">Crea Primo Verbale</button>
      </div>
    );
  }
  ...
  const polish = async (field: 'premis' | 'observations') => {
    ...
    try {
      ...
      const prompt = `Agisci come esperto collaudatore italiano. Riscrivi formalmente: "${currentDoc[field]}"`;
      ...
      <div className="space-y-6">
        {subTab === 'info' && (
          <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
            <h3 className="font-bold mb-6 flex items-center gap-2 uppercase text-xs tracking-widest text-slate-400 border-b pb-3"><Calendar className="w-4 h-4"/> 1. Informazioni Visita</h3>
        ...
        {subTab === 'convocation' && (
          <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
            <h3 className="font-bold mb-6 flex items-center gap-2 uppercase text-xs tracking-widest text-slate-400 border-b pb-3"><Mail className="w-4 h-4"/> 2. Convocazione</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div><label className="text-[10px] font-bold text-slate-500 uppercase">Metodo Invio</label><select className="w-full p-3 border rounded-xl bg-slate-50" value={currentDoc.convocationMethod} onChange={e => onUpdateDocument({...currentDoc, convocationMethod: e.target.value})}><option value="PEC">PEC</option><option value="Email">Email</option><option value="Nota Protocollata">Nota Protocollata</option><option value="Vie Brevi">Vie Brevi</option></select></div>
              ...
              <div className="md:col-span-2"><label className="text-[10px] font-bold text-slate-500 uppercase">Dettagli Aggiuntivi</label><input type="text" className="w-full p-3 border rounded-xl bg-slate-50" value={currentDoc.convocationDetails} onChange={e => onUpdateDocument({...currentDoc, convocationDetails: e.target.value})} placeholder="Es. via PEC del 10/10/2025" /></div>
            </div>
          </div>
        )}
        ...
        {subTab === 'attendees' && (
          <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
            <div className="flex justify-between items-center mb-6 border-b pb-3">
                <h3 className="font-bold flex items-center gap-2 uppercase text-xs tracking-widest text-slate-400"><Users className="w-4 h-4"/> 3. Soggetti Presenti</h3>
                <div className="flex gap-2">
                    ...
                    <button onClick={() => addAttendee(`per l'Impresa ${project.contractor.mainCompany.name}`, `${project.contractor.mainCompany.repTitle || 'Sig.'} ${project.contractor.mainCompany.repName}`)} className="px-2 py-1 bg-blue-50 border rounded text-[10px] font-bold text-blue-600">+ Impresa</button>
                </div>
            </div>
            <textarea className="w-full p-5 border rounded-2xl h-64 text-sm font-mono leading-relaxed bg-slate-50 shadow-inner" value={currentDoc.attendees} onChange={e => onUpdateDocument({...currentDoc, attendees: e.target.value})} placeholder="Elenco presenti..."/>
          </div>
        )}

        {subTab === 'works' && (
          <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
            <h3 className="font-bold mb-6 flex items-center gap-2 uppercase text-xs tracking-widest text-slate-400 border-b pb-3"><ListChecks className="w-4 h-4"/> 4. Lavorazioni Accertate</h3>
            <div className="mb-8">
            ...
            <div className="space-y-6">
                <div className="bg-slate-50 p-4 rounded-xl border border-dashed border-slate-200">
                    ...
                    <div className="flex gap-2 mb-4">
                        <input type="text" className="flex-1 p-2 border rounded-lg text-sm bg-white" value={newItem} onChange={e => setNewItem(e.target.value)} onKeyDown={e => e.key === 'Enter' && addItemToList('worksExecuted')} placeholder="Aggiungi lavorazione..."/>
                    ...
                    <ul className="space-y-2">
                        {currentDoc.worksExecuted.map((w, i) => (
                            <li key={i} className="flex justify-between items-center bg-white p-2 rounded border text-sm shadow-sm">
                                <input type="text" className="bg-transparent border-none flex-1 p-0 focus:ring-0" value={w} onChange={e => { const n = [...currentDoc.worksExecuted]; n[i] = e.target.value; onUpdateDocument({...currentDoc, worksExecuted: n}); }}/>
                                <button onClick={() => { const n = [...currentDoc.worksExecuted]; n.splice(i, 1); onUpdateDocument({...currentDoc, worksExecuted: n}); }} className="text-slate-300 hover:text-red-500"><Trash2 className="w-4 h-4"/></button>
                            </li>
                        ))}
                    </ul>
                </div>
                <div className="grid grid-cols-2 gap-4 border-t pt-4">
                    <div><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">In Corso</label><textarea className="w-full p-3 border rounded-xl h-32 text-sm bg-slate-50 shadow-inner" value={currentDoc.worksInProgress} onChange={e => onUpdateDocument({...currentDoc, worksInProgress: e.target.value})}/></div>
                    <div><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Prossime Attività</label><textarea className="w-full p-3 border rounded-xl h-32 text-sm bg-slate-50 shadow-inner" value={currentDoc.upcomingWorks} onChange={e => onUpdateDocument({...currentDoc, upcomingWorks: e.target.value})}/></div>
                </div>
            </div>
          </div>
        )}
        ...
        {['requests', 'invites', 'common'].includes(subTab) && (
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
                <h3 className="font-bold mb-6 flex items-center gap-2 uppercase text-xs tracking-widest text-slate-400 border-b pb-3">{subTab === 'requests' ? '5. Richieste' : subTab === 'invites' ? '6. Inviti' : '7. Parti Comuni'}</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-4">
                        <label className="text-[10px] font-bold text-slate-500 uppercase">Seleziona Frase Standard</label>
                        <select className="w-full p-3 border rounded-xl bg-slate-50 text-sm" onChange={e => setTempText(e.target.value === 'Altro...' ? '' : e.target.value)}><option value="">-- Scegli --</option>{OPTIONS[subTab as keyof typeof OPTIONS].map((opt, idx) => <option key={idx} value={opt}>{opt.substring(0, 60)}...</option>)}</select>
                        <div className="p-4 bg-blue-50 rounded-xl border border-blue-100">
                            ...
                            <button onClick={() => addToDocumentField(subTab === 'requests' ? 'testerRequests' : subTab === 'invites' ? 'testerInvitations' : 'commonParts', tempText)} className="mt-2 w-full bg-blue-600 text-white py-2 rounded-lg font-bold text-xs uppercase shadow-md">Aggiungi al Verbale</button>
                        </div>
                    </div>
                    <div className="space-y-4">
                        <label className="text-[10px] font-bold text-slate-500 uppercase">Contenuto Corrente</label>
                        <textarea className="w-full p-4 border rounded-2xl h-72 text-sm leading-relaxed bg-slate-50 font-serif shadow-inner" value={currentDoc[subTab === 'requests' ? 'testerRequests' : subTab === 'invites' ? 'testerInvitations' : 'commonParts']} onChange={e => onUpdateDocument({...currentDoc, [subTab === 'requests' ? 'testerRequests' : subTab === 'invites' ? 'testerInvitations' : 'commonParts']: e.target.value})}/>
                    </div>
                </div>
            </div>
        )}
        ...
        {subTab === 'evaluations' && (
          <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
            <div className="flex justify-between items-center mb-6 border-b pb-3"><h3 className="font-bold flex items-center gap-2 uppercase text-xs tracking-widest text-slate-400"><FileText className="w-4 h-4"/> 8. Valutazioni Conclusive</h3><button onClick={() => polish('observations')} disabled={isGenerating} className="text-xs text-purple-600 font-bold flex items-center gap-1 border border-purple-200 px-3 py-1 rounded-full hover:bg-purple-50">{isGenerating ? <Loader2 className="w-3 h-3 animate-spin"/> : <Wand2 className="w-3 h-3"/>} IA Formalizza</button></div>
            <textarea className="w-full p-6 border rounded-2xl h-96 text-sm leading-relaxed font-serif bg-slate-50 shadow-inner" value={currentDoc.observations} onChange={e => onUpdateDocument({...currentDoc, observations: e.target.value})} placeholder="Osservazioni conclusive..."/>
          </div>
        )}

        <div className="flex items-center justify-between p-4 bg-slate-900 rounded-2xl shadow-xl mt-8 text-white"><div className="text-[10px] font-bold uppercase tracking-widest flex items-center gap-2 opacity-80"><Save className="w-4 h-4 text-green-500"/> Salvataggio Database Locale</div><div className="text-[10px] font-bold uppercase tracking-widest opacity-30">Protocollo EdilApp v2.5</div></div>
      </div>
    </div>
  );
};
