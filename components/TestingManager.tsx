...
export const TestingManager: React.FC<TestingManagerProps> = ({
  ...
  if (!currentDoc) {
      return (
          <div className="flex flex-col items-center justify-center py-20 bg-white rounded-2xl border-2 border-dashed border-slate-200">
              ...
              <h3 className="text-xl font-bold text-slate-800">Archivio Atti di Collaudo</h3>
              <p className="text-slate-500 mb-6 text-center max-w-md">Seleziona un atto o creane uno nuovo per iniziare.</p>
  ...
  return (
    <div className="max-w-5xl mx-auto pb-20 animate-in fade-in">
       {/* Toolbar - Con controllo di sicurezza su .replace() */}
       <div className="flex items-center justify-between mb-8 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
            <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Atto Selezionato:</span>
                    <select 
                    ...
                    >
                      {documents.map(d => (
                        <option key={d.id} value={d.id}>
                            {(d.type || 'DOCUMENTO').replace(/_/g, ' ')} {' - '} {d.date ? new Date(d.date).toLocaleDateString() : 'N/D'}
                        </option>
                      ))}
                    </select>
                </div>
                ...
                <div className="relative">
                    ...
                    {showNewMenu && (
                        <div className="absolute right-0 top-full mt-2 w-64 bg-white border border-slate-200 rounded-xl shadow-2xl z-50 overflow-hidden">
                            ...
                            <button onClick={() => { onNewDocument('NULLA_OSTA_ENTE'); setShowNewMenu(false); }} className="w-full text-left p-3 hover:bg-slate-50 text-sm font-medium flex items-center gap-2"><FileSignature className="w-4 h-4 text-blue-500"/> Nulla Osta Incarico</button>
                        </div>
                    )}
                </div>
                {!readOnly && (
       ...
       <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 min-h-[500px]">
          {step === 'info' && (
              <div className="animate-in fade-in slide-in-from-right-4">
                  <h3 className="text-lg font-bold text-slate-800 mb-6 border-b pb-4 flex items-center gap-2"><Calendar className="w-5 h-5 text-blue-500"/> Inquadramento dell'Atto</h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      <div>
                          <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Data Atto / Sopralluogo</label>
                      ...
                      <div>
                          <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Numero Atto (I, II, III...)</label>
                          <input disabled={readOnly} type="number" className="w-full p-3 border border-slate-300 rounded-lg font-bold text-center focus:ring-2 focus:ring-blue-500/20" value={currentDoc.visitNumber || 1} onChange={e => onUpdateDocument({...currentDoc, visitNumber: parseInt(e.target.value) || 1})} />
                      </div>
                  </div>
              </div>
          )}
          ...
          {step === 'admin-acts' && isLettera && (
              <div className="animate-in fade-in slide-in-from-right-4 space-y-8">
                  <h3 className="text-lg font-bold text-slate-800 mb-4 border-b pb-2 flex items-center gap-2"><Layout className="w-5 h-5 text-blue-500"/> Destinatari e Layout</h3>
                  <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase mb-2 tracking-widest">Blocco Destinatari (Testo Libero)</label>
                      <textarea disabled={readOnly} className="w-full p-4 border border-slate-300 rounded-lg h-48 text-sm font-mono leading-tight bg-slate-50 focus:bg-white transition-all shadow-inner" 
                          placeholder={"Inserisci i destinatari...\nEs. Spett.le Comune di..."}
                          value={currentDoc.actRecipientsBlock || ""} 
                          onChange={e => onUpdateDocument({...currentDoc, actRecipientsBlock: e.target.value})} />
                  </div>
                  <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase mb-2 tracking-widest">Corpo della Lettera</label>
                      <textarea disabled={readOnly} className="w-full p-4 border border-slate-300 rounded-lg h-72 text-sm leading-relaxed bg-slate-50 focus:bg-white transition-all shadow-inner" 
                          value={currentDoc.actBodyOverride || ""} 
                          onChange={e => onUpdateDocument({...currentDoc, actBodyOverride: e.target.value})} 
                          placeholder="Inserisci il testo della lettera o lascia vuoto per il default." />
                  </div>
              </div>
          )}
          ...
          {step === 'works' && isVerbale && (
              <div className="animate-in fade-in slide-in-from-right-4 space-y-6">
                  <h3 className="text-lg font-bold text-slate-800 mb-4 border-b pb-2 flex items-center gap-2"><ListChecks className="w-5 h-5 text-blue-500"/> Premesse Storiche</h3>
                  <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase mb-2 tracking-widest">Testo Premesse</label>
                      <textarea disabled={readOnly} className="w-full p-4 border border-slate-300 rounded-lg h-[400px] text-sm leading-relaxed bg-slate-50 focus:bg-white transition-all shadow-inner font-mono" 
                          value={currentDoc.premis || ""} 
                          onChange={e => onUpdateDocument({...currentDoc, premis: e.target.value})} 
                          placeholder="Riepilogo dei verbali e atti precedenti..." />
                  </div>
              </div>
          )}
       </div>
