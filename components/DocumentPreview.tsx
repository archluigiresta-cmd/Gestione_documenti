...
export const DocumentPreview: React.FC<DocumentPreviewProps> = ({ project, doc }) => {
  ...
  // Correzione Nomenclatura Cablata
  const typeLabel = "VERBALE DI VISITA DI COLLAUDO";
    ...
    <div id="document-preview-container" className="font-serif-print text-black leading-snug w-full max-w-[21cm] mx-auto bg-white">
      <div className="p-[1.8cm] min-h-[29.7cm] flex flex-col border-none">
        ...
        <div style={{ borderTop: '1.5pt solid black', paddingTop: '10pt', marginBottom: '25pt' }}>
            <p style={{ fontSize: '10pt', margin: '0 0 12pt 0', fontWeight: 'bold' }}>CUP: {project.cup || '---'} - CIG: {project.cig || '---'}</p>
        ...
        <table style={{ width: '100%', border: '1pt solid black', borderCollapse: 'collapse', fontSize: '11pt', marginBottom: '30pt' }}>
            <tbody>
                ...
                <tr><td style={{ border: '1pt solid black', padding: '8pt', fontWeight: 'bold', backgroundColor: '#f9fafb' }}>Importo Contrattuale:</td><td style={{ border: '1pt solid black', padding: '8pt' }}>{formatCurrency(project.contract?.totalAmount)} oltre IVA</td></tr>
        ...
        <div style={{ textAlign: 'justify', fontSize: '12pt', lineHeight: '1.7' }}>
            ...
            <p style={{ margin: '15pt 0 12pt 0' }}>{doc.worksIntroText}</p>
            ...
            {doc.testerRequests && (
                <div style={{ marginTop: '20pt' }}>
                    <p style={{ fontWeight: 'bold' }}>Il Collaudatore chiede:</p>
                    <div style={{ whiteSpace: 'pre-wrap', paddingLeft: '15pt', borderLeft: '1pt solid #eee' }}>{doc.testerRequests}</div>
                </div>
            )}
            
            {doc.testerInvitations && (
                <div style={{ marginTop: '20pt' }}>
                    <p style={{ fontWeight: 'bold' }}>Il Collaudatore invita:</p>
                    <div style={{ whiteSpace: 'pre-wrap', paddingLeft: '15pt', borderLeft: '1pt solid #eee' }}>{doc.testerInvitations}</div>
                </div>
            )}
            ...
            {doc.observations && (
                <div style={{ marginTop: '25pt', padding: '10pt', border: '0.5pt dashed #ccc' }}>
                    <p style={{ fontWeight: 'bold', marginBottom: '5pt' }}>VALUTAZIONI:</p>
                    <div style={{ whiteSpace: 'pre-wrap', fontStyle: 'italic' }}>{doc.observations}</div>
                </div>
            )}
        </div>

        <div style={{ marginTop: 'auto', paddingTop: '60pt', pageBreakInside: 'avoid' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '30pt' }}>
                <div style={{ marginBottom: '30pt' }}>
                    ...
                    <p style={{ fontSize: '11pt' }}>{tester.title} {tester.name}</p>
                    <p style={{ marginTop: '20pt' }}>_____________________________</p>
                </div>
                <div style={{ marginBottom: '30pt' }}>
