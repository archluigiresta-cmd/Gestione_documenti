...
const App: React.FC = () => {
  ...
  const loadProjects = async () => {
    ...
    setProjectList(projects.sort((a, b) => a.displayOrder - b.displayOrder));
  };
      ...
      <div className="flex bg-slate-100 min-h-screen">
        ...
        <main className="ml-64 flex-1 p-8 h-screen overflow-y-auto print:ml-0 print:p-0">
          {['general', 'design', 'subjects', 'tender', 'contractor'].includes(activeTab) && (
            <ProjectForm data={currentProject} onChange={handleProjectUpdate} section={activeTab as any} readOnly={userRole === 'viewer'} />
          )}
          {activeTab === 'execution' && (
            <ExecutionManager project={currentProject} onUpdateProject={handleProjectUpdate} documents={documents} currentDocId={currentDocId} onSelectDocument={setCurrentDocId} onUpdateDocument={handleDocumentUpdate} onNewDocument={() => {}} onDeleteDocument={() => {}} readOnly={userRole === 'viewer'} />
          )}
          {activeTab === 'testing' && (
            <TestingManager project={currentProject} documents={documents} currentDocId={currentDocId} onSelectDocument={setCurrentDocId} onUpdateDocument={handleDocumentUpdate} onNewDocument={() => {}} onDeleteDocument={() => {}} readOnly={userRole === 'viewer'} onUpdateProject={handleProjectUpdate} />
          )}
          {activeTab === 'export' && (
