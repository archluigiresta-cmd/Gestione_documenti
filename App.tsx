        ...
        <div className="flex bg-slate-100 min-h-screen">
          ...
          <main className="ml-64 flex-1 p-8 h-screen overflow-y-auto">
              {['general', 'design', 'subjects', 'tender', 'contractor'].includes(activeTab) && (
                <ProjectForm data={currentProject} onChange={p => db.saveProject(p)} section={activeTab as any} readOnly={userRole === 'viewer'} />
              )}
              {activeTab === 'execution' && (
                <ExecutionManager project={currentProject} onUpdateProject={p => db.saveProject(p)} documents={documents} currentDocId={currentDocId} onSelectDocument={setCurrentDocId} onUpdateDocument={d => db.saveDocument(d)} onNewDocument={() => {}} onDeleteDocument={() => {}} readOnly={userRole === 'viewer'} />
              )}
              {activeTab === 'testing' && (
                <TestingManager project={currentProject} documents={documents} currentDocId={currentDocId} onSelectDocument={setCurrentDocId} onUpdateDocument={d => db.saveDocument(d)} onNewDocument={() => {}} onDeleteDocument={() => {}} readOnly={userRole === 'viewer'} />
              )}
              {activeTab === 'export' && (
